...
void BM_Network::PhysicalController::read() {
    ...
    while (true) {
        ...
        if (n > 0) {
            buf[n] = 0;
            datalink_controller.sendData((char *) buf);
        }
#ifdef _WIN32
        Sleep(100);
#else
...
void BM_Network::PhysicalController::sendData(const BM_Network::byte *data) {
...
void BM_Network::DatalinkControllerPL::sendData(const byte *data) {
    datalink_controller_impl->sendData(data);
}